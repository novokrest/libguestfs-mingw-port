...
struct connection_socket {
    ...
    os_socketpair_end_t console_sock;          /* Appliance console (for debug info). */
    os_socket_t daemon_sock;         /* Daemon communications socket. */
...
accept_connection (guestfs_h *g, struct connection *connv)
{
  ...
  while (sock == INVALID_SOCKET) {
    ...
    /* Log message? */
    if (conn->console_sock) {
      bSuccess = PeekNamedPipe(conn->console_sock, NULL, 0, NULL, &cbConsoleAvail, NULL);
  ...
  while (len > 0) {
    ...
    if (FD_ISSET(conn->daemon_sock, &readfds)) {
      ...
      closed:
         ...
         * them out here.
         */
        if (g->verbose && conn->console_sock) {
                    ...
                    struct connection_socket *conn)
{
  ...
  bSuccess = ReadFile(conn->console_sock, buf, BUFSIZ, &dwRead, NULL);
